...
class PathRequestTest < MiniTest::Test
  ...
  def test_counter_goes_up_when_greeting_is_called
  ...
  def test_greeting_says_hello
    ...
    assert_equal "Hello, World! (1)", path.greeting
  end

  def test_hello_path_returns_greeting
    path = PathRequest.new

    assert_equal "Hello, World! (1)", path.paths("/hello", [], @response_body)
  end

  def test_datetime_path_returns_date
    path = PathRequest.new

    assert_equal Time.now.strftime('%a, %e %b %Y %H:%M:%S %z'), path.paths("/datetime", [], @response_body)
  end

  def test_shutdown_returns_total_requests
    path = PathRequest.new
    47.times do
      path.paths("/hello", [], @response_body)
    end
    assert_equal "Total requests: 47", path.paths("/shutdown", [], @response_body)
  end
  ...
  def test_word_search_returns_whether_words_are_known_or_not
    ...
    assert_equal result, path.paths("/word_search", ["hello", "pizza", "ajdlkfjal"], @response_body)
  end
  ...
  def test_blank_path_returns_response_body
    ...
    assert_equal result, path.paths("/", [], @response_body)
  end
  ...
  def test_different_responses_with_multiple_requests
    ...
    assert_equal result1, path.paths("/datetime", [], @response_body)
    assert_equal result2, path.paths("/", [], @response_body)
    assert_equal result3, path.paths("/hello", [], @response_body)
  end
end
